...
## Keyboard & gamepad
- **Keyboard:** `F11` fullscreen, `Esc` exit Immersive, `Tab`/`Shift+Tab` cycle views, arrows move inside per-page grid/list handlers, `Enter`/`Space` activate. Immersive Mode is entered from the Power menu only (no keyboard shortcut).
...
## WiFi integration
- **Status chip in the TopBar** + **picker modal** in the Moonlight page (one tap to connect/scan/disconnect/toggle the radio). Two visual states per network row: **Connected** (Disconnect button) or **Saved / unknown** (whole row is the action, click anywhere).
- **Rust `wifi.rs` is netsh-based**, not wlanapi. Read-side wlanapi calls (`WlanEnumInterfaces` / `WlanQueryInterface` / `WlanGetAvailableNetworkList`) return `ERROR_NOT_FOUND` (1168) inside a long-lived Tauri process even though the same calls work from `cargo run` — the wlan service's per-client cache goes stale. So:
  - `wifi_current` shells out to `netsh wlan show interfaces` and parses `State` / `SSID` / `Signal` / `Authentication` / `Radio status`. Returns `Some(...)` when the radio is on (even if not connected) and `null` when there's no adapter — the chip is hidden on desktops / VMs.
  - `wifi_scan` calls `WlanScan` (the wlanapi write opcode, which works in the Tauri process), sleeps ~2.5s to let the driver populate the visible-network cache, then `netsh wlan show networks mode=bssid` and parses `SSID N` blocks. Each block's BSSIDs give the strongest `Signal` (max), `connected` flag (vs. `netsh_current_connection`), and best `gen` (`Radio type` line → 4/5/6/7). The first `netsh show networks` call returns only the connected network; the second call after the sleep returns the full visible list — that's how the cache populates. `wifi_scan` is registered as `async` and dispatched via `tauri::async_runtime::spawn_blocking` so other Tauri commands (the chip's poll, connect, disconnect) interleave normally while a scan is in flight.
  ...
  - `wifi_radio_get()` reads via wlanapi (the radio-state opcode doesn't hit the 1168 bug). `wifi_radio_set(enabled)` toggles via `WlanSetInterface`, falling back to `netsh interface set interface "Wi-Fi" admin=...` if the wlanapi call fails.
- **`useWifi` (hook)** is event-driven, not a 30s tick. Reads on mount, on window `focus`, on `visibilitychange` → visible, and on an explicit `refresh()` call from the parent. Two refs collapse back-to-back events: `inFlight` (the in-progress Promise — concurrent callers share the same read) and `lastReadAt` (skip if a read completed within 2s). The chip and the modal share a single `useWifi` instance via prop drilling; on `wifiOpen` change the parent calls `refreshWifi()` so the chip reflects any change the user just made in the modal.
- **WiFi picker modal** in the Moonlight page (`src/components/ui/WifiModal.tsx`):
  ...
  - **Per-row in-flight UI.** Each row has its own `busy` state (`{ ssid, kind: "connect" | "disconnect" } | null`). The busy row shows an inline spinner + "Connecting…" / "Disconnecting…" subtitle; all other rows are disabled via a separate `anyBusy` prop. The connected row dims to opacity-40 during disconnect to match the connecting treatment.
  ...
  - **Radio toggle** in the footer is the existing `Toggle` component. Rescan button on the left.
