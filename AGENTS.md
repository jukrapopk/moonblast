...
- **Realtime updates** without Rust push events: `src/hooks/useBattery.ts` mirrors `useWifi`'s `inFlight`/`lastReadAt(2s)` collapse pattern, with a **5s background poll** plus refresh on `focus` and `visibilitychange` → visible. The modal adds its own **2s refresh** while open so the percent and time-remaining tick down live. The OS itself only updates `SYSTEM_POWER_STATUS` at ~1Hz, so faster polling would just return duplicate values; the 5s cadence matches that.
