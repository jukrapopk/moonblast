...
- **WiFi picker modal** (`src/components/ui/WifiModal.tsx`):
  ...
  - **Footer.** Rescan button on the left (disabled while the radio is off); "WiFi Settings" text button on the right opens Windows Wi-Fi settings (radio on/off lives in the OS).
...
## Audio integration
- **Speaker chip in the TopBar** (right of WiFi) + **mixer modal** (`src/components/ui/AudioModal.tsx`): output-device picker, main volume slider + mute, per-app sliders (grouped by exe like the Windows mixer) each with mute + reset-to-max, a "Reset all" button (every app channel to max + unmuted), Refresh, and a "Sound Settings" footer button (`open_sound_settings` → `ms-settings:sound`).
...
- **`useAudio` (hook)**: `useAudioMaster` for the chip (mount / focus / visible / explicit refresh + a slow 5s poll to catch volume-key / external-mixer changes — the COM read is ~1ms). Slider drags invoke fire-and-forget (no await per tick); mute/reset await + refresh. Parent refreshes the chip on modal close.
