...
- **Auto Immersive Mode** — one Settings toggle that makes Moonblast your Windows shell: signing in goes straight into the launcher, fullscreen, instead of the desktop. Registered per-user (`Winlogon\Shell`), so there's **no UAC prompt, ever**. Explorer never starts, which means no desktop/taskbar flash and no startup apps at all (Explorer is what runs them). Toggling it only arms the *next* sign-in — it never drops you into Immersive Mode on the spot. Exiting Immersive Mode or closing Moonblast hands the desktop back; a stub supervisor guarantees a working desktop even if Moonblast crashes. The stub itself exits cleanly when the launcher exits with code 0 (user-initiated close), so closing Moonblast doesn't leave a stray no-window stub in Task Manager for the rest of the session. Upgrading from a build with the old "Start with Windows" toggle deletes the stale `HKCU\…\Run\Moonblast` value on first boot so the Run key and stub don't double-launch Moonblast at sign-in.
...
- **Rust backend** (`src-tauri/src/lib.rs`) exposes Tauri commands for:
  ...
  - startup: `apply_auto_immersive(bool)` — registers or unregisters Moonblast as the per-user `Winlogon\Shell` (the only boot-launch path; no Run key to coordinate). `booted_as_shell` reports whether this process was started by the shell stub at sign-in.
